...
async function main() {
    ...
    writeFileSync(
        ...
        PY_TEMPLATE.replace("{{ version }}", version)
            ...
            .replace("{{ protos_equals }}", pyEquals),
    );
    ...
    // Compile the protos directly so this script is self-contained.
    const protoArgs = filenames.join(" ");
    try {
        execSync(`protoc --proto_path=./discord_protos/ --python_out=discord_protos ${protoArgs}`, { cwd: root, stdio: "inherit" });
    } catch (e) {
